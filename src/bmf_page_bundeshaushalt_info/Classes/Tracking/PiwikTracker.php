...
*/
    protected function getCookieName($cookieName)
    {
        ...
        $cookieDomain = $this->configCookieDomain === '' ? self::getCurrentHost() : $this->configCookieDomain;
        $hash = substr(
            sha1($cookieDomain . $this->configCookiePath),
            0,
            4
        );
...
* @param bool|int $countResults (optional) results displayed on the search result page. Used to track "zero result"
*                               keywords.
*
...
* @param string|array $category (optional) Product category, or array of product categories (up to 5 categories can be
...
*
* If the Ecommerce order contains items (products), you must call first the addEcommerceItem() for each item in the
* order. All revenues (grandTotal, subTotal, tax, shipping, discount) will be individually summed and reported in Piwik
* reports. Only the parameters $orderId and $grandTotal are required.
*
* @param string|int $orderId (required) Unique Order ID.
*                This will be used to count this order only once in the event the order page is reloaded several times.
*                orderId must be unique for each transaction, even on different days, or the transaction will not be
*                recorded by Piwik.
* @param float $grandTotal (required) Grand Total revenue of the transaction (including tax, shipping, etc.)
* @param float $subTotal (optional) Sub total amount, typically the sum of items prices for all items in this order
* (before Tax and Shipping costs are applied)
...
* conversion rates (Conversion rate = Ecommerce orders containing this product or category /
* Visits to the product or category)
*
...
*               tracking_requests_require_authentication_when_custom_timestamp_newer_than), then you must call
*               setTokenAuth() with a valid Admin/Super user token.
* @return $this
*/
...
*
* A User ID can be a username, UUID or an email address, or any number or string that uniquely
* identifies a user or client.
*
* @param string $userId Any user ID string (eg. email address, ID, username). Must be non empty.
*                       Set to false to de-assign a user id previously set.
* @return $this
* @throws Exception
*/
